                    ...
                    if(rndInt !== 'undefined' && rndInt < total_sites && total_sites > 0){
                        ...
                        const send = async () => {
                            ...
                            await producer.send({
                                  topic: 'uk',
                                  messages: [
                                      {partition: mod ,value: JSON.stringify({ "bookmaker": matches[count].sites[rndInt].site_nice,"team1" : matches[count].teams[0], "team2" :matches[count].teams[1],  "odds1" : matches[count].sites[rndInt].odds['h2h'][0], "odds2" : matches[count].sites[rndInt].odds['h2h'][1] }) },
                                  ],
                            })

                            //console.log("message is sent from pub 1") 
