...
class OddsRoom extends React.Component {
	constructor(props){
		...
		this.state = {
			...
			msgs: [],
			selectedValues : [],
      options: [{ publisher:'pub1' ,topic: 'us', id: 1,},{ publisher:'pub2', topic: 'uk', id: 2},{ publisher:'pub3', topic: 'au', id: 3},{ publisher:'pub4', topic: 'eu', id: 4}]
		}
		...
		return(
            ...
            <Container className="event-header-container">
						...
						{this.state.msgs.map((item , index) => {
							...
							return(<Container key={index} className="event-container">
									   <Row className="p-4">
									        <Col xs={2} className="p-2">
									          {item.bookmaker}
									        </Col>
