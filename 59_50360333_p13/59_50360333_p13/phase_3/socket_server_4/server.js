const server = require("socket.io")(7004,{
  cors: {
    origin: '*',
  }
});
...
const node1_socket = io_client.connect("ws://localhost:7001");
...
node1_socket.on("subscribe",(topic,publisher,data) => {
  ...
  if(sub_serving1 == 0){
    ...
    if(myTopics.includes(r_topic)){
      ...
      console.log("subscription request was reached at node 4");

      server.broadcast.emit("subscribe", r_topic , r_publisher, r_data);
...
node1_socket.on("unsubscribe",(topic,publisher,data) => {
  ...
  if(unsub_serving1 == 0){
    ...
    if(myTopics.includes(r_topic)){
          console.log("subscriber ", r_data.name, " has been removed from node 4 for topic ", r_topic);
...
server.on("connection", socket => {
  ...
  console.log("someone connected to node4");
  ...
  socket.on("subscribe",(topic,publisher,data) => {
    ...
    if(dir_serving == 0){
      ...
      // emit meesages to subscribed clients
      socket.broadcast.emit(r_topic,r_data);

      // forward message to neighbor broker server
      for (const [key, value] of nodeTopicsMap.entries()) {
          key.emit("publish", r_publisher, r_topic , r_data);
          console.log("forwarded publish requests");
      }

      // forward message to connected client brokers
  ...
  socket.on("unsubscribe",(topic,publisher,data) => {
    ...
    if(unsub_serving == 0){
      ...
      if(myTopics.includes(r_topic)){
        ...
        console.log("unsubscription request was reached at node 4");

        for (const [key, value] of nodeTopicsMap.entries()) {
            key.emit("unsubscribe", r_topic , r_publisher, r_data);
            console.log("forwarded unsub requests from node 4");
        }
