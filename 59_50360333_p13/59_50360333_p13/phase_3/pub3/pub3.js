...
function advertise(topic){
    console.log("advertising au topic");
...
function publish(topic, event){
    socket.emit("publish", "pub3", topic, event);
}
            ...
            if(count < match_count){     
               ...
               try{
                    ...
                    if(rndInt !== 'undefined' && rndInt < total_sites && total_sites > 0){
                       ...
                       publish("au", { "bookmaker": matches[count].sites[rndInt].site_nice,"team1" : matches[count].teams[0], "team2" :matches[count].teams[1],  "odds1" : matches[count].sites[rndInt].odds['h2h'][0], "odds2" : matches[count].sites[rndInt].odds['h2h'][1] });
                    }
                }
                catch(e){
                    console.log(e)
