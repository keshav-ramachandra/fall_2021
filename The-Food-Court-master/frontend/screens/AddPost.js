...
const AddPost = (props, navigation) => {
 ...
 const expoNotify = (expo_ids) => {
        let text = emailId.split("@")[0] + " has tagged you in a post which needs approval from admin"
 ...
 const sendNotifications = async (users) => {
    ...
    for(var i=0;i<users.length;i++){
      u_names.push(users[i]['item'])
    }
    console.log("u naaaaaa", u_names)
      ...
      try {
        ...
        axios.post(server + `posts/savePost`, JSON.stringify(User))
          .then(async(res) => {
            ...
            sendNotifications(selectFriend)
            setSelectedFoodTag([])
            setSelectFriend([])
